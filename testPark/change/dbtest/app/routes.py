...
def result():
    try:
        ...
        # 날씨 아이콘 URL 결정 (2024.11.20 hourly_data 위로 올려야 함!!)
        ...
        for item in items:
            if item['category'] == 'TMP':  # 기온 데이터만 추출
                ...
                # 해당 시간의 sky_status를 추출
                sky_status_code = next(
                    (sky['fcstValue'] for sky in items if sky['category'] == 'SKY' and sky['fcstTime'] == fcst_time),
                    None
                )
                sky_status_str_hourly = sky_status_map.get(str(sky_status_code), "알 수 없음")  # 시간별 상태

                hourly_data.append({
                    "time": fcst_time,
                    "temperature": temperature,
                    "sky_status": sky_status_str_hourly,
                    "icon": get_weather_icon(sky_status_str_hourly)  # 아이콘 추가
                })
